...
public class WeekView extends View {
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    if (bottom > mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 && left < right &&
                        ...
                        drawText(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, top, left);
                    }
                    else
     ...
     * @param event The event which title should be draw
     * @param rect The rectangle on which the text is to be drawn.
     * @param canvas The canvas to draw upon.
     * @param originalTop The original top position of the rectangle. The rectangle may have some of its portion outside of the visible area.
     * @param originalLeft The original left position of the rectangle. The rectangle may have some of its portion outside of the visible area.
     */
    private void drawText(Eventable event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
